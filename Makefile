...
release:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build $(BUILD_FLAGS) -o dist/linux_amd64/netkit ./cmd/netkit
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build $(BUILD_FLAGS) -o dist/linux_arm64/netkit ./cmd/netkit
	tar -C dist/linux_amd64 -czf dist/netkit_linux_amd64.tar.gz netkit
	tar -C dist/linux_arm64 -czf dist/netkit_linux_arm64.tar.gz netkit
