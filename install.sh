...
repository="${NETKIT_REPOSITORY:-rustyllh/netkit}"
install_dir="${NETKIT_INSTALL_DIR:-/usr/local/bin}"
version="${NETKIT_VERSION:-}"
bash_completion_loader="${NETKIT_BASH_COMPLETION_LOADER:-/usr/share/bash-completion/bash_completion}"
bash_completion_dir="${NETKIT_BASH_COMPLETION_DIR:-/usr/share/bash-completion/completions}"
bashrc="${NETKIT_BASHRC:-/root/.bashrc}"
...
release_url="${NETKIT_RELEASE_BASE_URL:-https://github.com/${repository}/releases/download}/${version}"
...
checksum_file="${temporary_dir}/${package}.sha256"
grep -F "  ${package}" "${temporary_dir}/SHA256SUMS" > "$checksum_file" || fail "SHA256SUMS 中缺少 ${package}"
[ -s "$checksum_file" ] || fail "SHA256SUMS 中缺少 ${package}"
(
	cd "$temporary_dir"
...
configure_bash_completion() {
	if [ ! -f "$bash_completion_loader" ]; then
...
install_completion "${temporary_dir}/completions/netkit.bash" "$bash_completion_dir" "netkit"
